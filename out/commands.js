...
class CommandCenter {
    constructor(bzr, model, outputChannel) {
        this.bzr = bzr;
        this.model = model;
    ...
    init() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const path = yield vscode_1.window.showInputBox({
                ...
                prompt: localize('provide path', "Please provide a folder path to initialize a Bzr repository"),
                value,
                ignoreFocusOut: true
            });
            if (!path) {
    ...
    createCommand(id, key, method, options) {
        const result = (...args) => {
            ...
            return result.catch((err) => __awaiter(this, void 0, void 0, function* () {
                let message;
                switch (err.bzrErrorCode) {
                    //case BzrErrorCodes.DirtyWorkTree:
                    //  message = localize('clean repo', "Please clean your repository working tree before checkout.");
                    //  break;
                    //case BzrErrorCodes.PushRejected:
                    ...
                    default:
                        const hint = (err.stderr || err.message || String(err))
                            ...
                            .filter(line => !!line)[0];
                        message = hint
                            ? localize('bzr error details', "Bzr: {0}", hint)
                            : localize('bzr error', "Bzr error");
                        break;
                }
                if (!message) {
                    console.error(err);
                ...
                const openOutputChannelChoice = localize('open bzr log', "Open Bzr Log");
